...
export default function TestUiPage() {
  ...
  const handleCreateDocument = async () => {
    const result = await apiRequest<{ id: string }>("/api/documents", {
      method: "POST",
  ...
  const handleListDocuments = async () => {
    const result = await apiRequest<DocumentRecord[]>("/api/documents", undefined, token);
    appendLog("listDocuments", result);
  };

  const handleGetDocument = async () => {
    const result = await apiRequest<DocumentRecord>(`/api/documents/${documentId}`, undefined, token);
    appendLog("getDocument", result);
  };

  const handleUpdateDocument = async () => {
    const result = await apiRequest<null>(`/api/documents/${documentId}`, {
      method: "PUT",
      body: JSON.stringify(documentForm),
    }, token);
  ...
  const handleDeleteDocument = async () => {
    const result = await apiRequest<null>(`/api/documents/${documentId}`, { method: "DELETE" }, token);
  ...
  const handleSearchDocuments = async () => {
    ...
    const result = await apiRequest<DocumentRecord[]>("/api/documents/search", {
      method: "POST",
  ...
  const handleRecommend = async () => {
    const result = await apiRequest<RecommendOutput>("/api/recommend", {
      method: "POST",
    ...
    <main style={styles.page}>
      ...
      <section style={styles.grid}>
        <div style={styles.card}>
          ...
          <input style={styles.input} placeholder="username" value={signupForm.username}
            onChange={(e) => setSignupForm((p) => ({ ...p, username: e.target.value }))} />
          <input style={styles.input} placeholder="email" value={signupForm.email}
            onChange={(e) => setSignupForm((p) => ({ ...p, email: e.target.value }))} />
          <input style={styles.input} placeholder="password" type="password" value={signupForm.password}
          ...
          <input style={styles.input} placeholder="password" type="password" value={loginForm.password}
            onChange={(e) => setLoginForm((p) => ({ ...p, password: e.target.value }))} />
          <button style={styles.buttonSecondary} onClick={handleLogin}>Login</button>
          <textarea style={styles.textarea} value={token}
            onChange={(e) => setToken(e.target.value)} placeholder="JWT token" rows={4} />
        </div>

        <div style={styles.card}>
          <h2 style={styles.cardTitle}>Documents</h2>
          <input style={styles.input} placeholder="title" value={documentForm.title}
            onChange={(e) => setDocumentForm((p) => ({ ...p, title: e.target.value }))} />
          <textarea style={styles.textarea} placeholder="content" value={documentForm.content}
            onChange={(e) => setDocumentForm((p) => ({ ...p, content: e.target.value }))} rows={4} />
          ...
          <input style={styles.input} placeholder="document id" value={documentId}
        ...
        <div style={styles.card}>
          ...
          <textarea style={styles.textarea} placeholder="0.1,0.2,0.3" value={searchEmbedding}
            onChange={(e) => setSearchEmbedding(e.target.value)} rows={4} />
          <button style={styles.button} onClick={handleSearchDocuments}>Vector Search</button>
          <input style={styles.input} placeholder="recommend query" value={recommendQuery}
...
const styles: Record<string, React.CSSProperties> = {
  ...
  logCard: { maxWidth: 1100, margin: "16px auto 0", background: "#0f172a", borderRadius: 24, padding: 20, color: "#e2e8f0" },
  ...
  log: { overflowX: "auto", whiteSpace: "pre-wrap", wordBreak: "break-word", margin: 0 },
};
