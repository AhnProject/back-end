...
import com.devahn.support.util.VectorUtils;
...
public class DocumentRepositoryImpl implements DocumentRepository {
    ...
    public Long save(Document document) {
        String sql = "INSERT INTO documents(title, content, embedding, created_at) VALUES (?, ?, ?::vector, now())";
    ...
    public Optional<Document> findById(Long id) {
        String sql = "SELECT id, title, content, embedding, created_at, updated_at FROM documents WHERE id = ?";
    ...
    public List<Document> findAll() {
        String sql = "SELECT id, title, content, embedding, created_at, updated_at FROM documents ORDER BY created_at DESC";
        return jdbcTemplate.query(sql, documentRowMapper);
    }

    @Override
    public int update(Document document) {
        String sql = "UPDATE documents SET title = ?, content = ?, embedding = ?::vector, updated_at = now() WHERE id = ?";
        float[] embedding = VectorUtils.adjustVectorDimension(document.getEmbedding(), VECTOR_DIM);
        String vectorString = VectorUtils.vectorToString(embedding);
        return jdbcTemplate.update(sql, document.getTitle(), document.getContent(), vectorString, document.getId());
    }
    ...
    public List<Document> searchByVector(float[] queryVector, int limit) {
        ...
        return jdbcTemplate.query(sql, documentRowMapper, vectorString, limit);
    }
}
